...
You are given a 0-indexed integer array vals of length n where vals[i] denotes the value of the ith node. You are also given a 2D 
...
For 
�
T operations, the amortized time complexity of the union-find algorithm (using path compression with union by rank) is 
�
(
�
�
...
α(T) is the inverse Ackermann function that grows so slowly, that it doesn't exceed 
4
...
T<10 
600
 ). You can read more about the complexity of union-find here. Because the function grows so slowly, we consider it to be 
�
(
1
)
O(1). We iterate over each edge once from the larger value node to the smaller one, or if the neighbours (nodes that share an edge) have equal value, we iterate that edge twice, which is also linear. To iterate over 
�
−
1
n−1 edges, we have to perform 
�
(
�
)
O(n) operations which needs 
�
(
�
)
O(n) time.
We also need a map valuesToNodes having sorted keys. Each operation in such a data structure comes with a 
�
�
�
log factor. We push all the n nodes into the map and iterate over all of them, which further adds 
�
(
�
⋅
...
O(n⋅log(n))) time.
The group map has unsorted keys, and each of its operation takes 
�
(
1
)
O(1) time on average. We need 
�
(
�
)
O(n) time to iterate through all of the nodes to find the set size using it.
Additionally, we need 
�
(
�
)
O(n) time each to initialize the adj, parent and rank arrays.
Space complexity: 
�
(
�
...
O(n) space each for the adj, parent and rank arrays.
We also require 
�
(
�
)
O(n) space for the valuesToNodes and the group maps.
 */